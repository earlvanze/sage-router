...
readable or creatable. If `canApplyExistingCandidate` is `false`, rotate or
...
return `401` with account, setup-key recovery, pricing, status, OpenAI base URL,
and `sk_sage_` setup guidance.
