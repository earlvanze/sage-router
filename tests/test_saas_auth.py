...
class SaaSAuthTests(unittest.TestCase):
    ...
    def test_activation_auth_repair_handoff_is_aggregate_only(self):
        handoff = router.launch_activation_auth_repair_handoff(
            [
                {
        ...
        self.assertEqual('review_required', handoff['status'])
    ...
    def test_launch_funnel_snapshot_seeds_acquisition_actions_without_traffic(self):
        ...
        self.assertTrue(all(row['secretFree'] for row in snapshot['acquisitionActions']))
        self.assertTrue(all(row['publicSafe'] for row in snapshot['acquisitionActions']))
        self.assertTrue(all(row['sendsEmail'] is False for row in snapshot['acquisitionActions']))
        self.assertTrue(all(row['mutatesRuntime'] is False for row in snapshot['acquisitionActions']))
        self.assertTrue(all(row['managedResaleEnabled'] is False for row in snapshot['acquisitionActions']))
        self.assertTrue(all('/' in row['ctaPath'] for row in snapshot['acquisitionActions']))
        self.assertTrue(all(not row['privacy']['containsEmails'] for row in snapshot['acquisitionActions']))
    ...
    def test_launch_funnel_snapshot_counts_private_conversion_stages(self):
        ...
        router.read_launch_marketing_funnel_counts = lambda _since, limit=10000: ({
            ...
            'founderSalesOutreachCopies': 2,
            'founderSalesOutreachCopiesBySnippet': {
                'founder-one-subscription-reply': 1,
                'founder-recommended-first-reply-email-draft': 1,
            },
        ...
        self.assertEqual(31, snapshot['stages']['marketingIntentEvents'])
        ...
        self.assertEqual(1, snapshot['marketingIntent']['events']['outreach_email_draft_opened'])
        ...
        self.assertEqual(2, snapshot['marketingIntent']['founderSalesOutreachCopies'])
        self.assertEqual(1, snapshot['marketingIntent']['founderSalesOutreachCopiesBySnippet']['founder-one-subscription-reply'])
        self.assertEqual(1, snapshot['marketingIntent']['founderSalesOutreachCopiesBySnippet']['founder-recommended-first-reply-email-draft'])
        ...
        self.assertEqual(2, snapshot['marketingIntent']['sourceSurfaces']['founder-sales'])
        ...
        self.assertEqual('approve_after_review', approval['decisionLines'][0]['id'])
        self.assertIn('APPROVE_ACTIVATION_FOLLOWUP', approval['decisionLines'][0]['value'])
        self.assertIn('HOLD_ACTIVATION_FOLLOWUP', approval['decisionLines'][1]['value'])
        self.assertFalse(approval['decisionLines'][0]['mutatesRuntime'])
        self.assertFalse(approval['decisionLines'][0]['sendsEmail'])
        ...
        self.assertEqual(snapshot['mrr']['planRevenueActions'], snapshot['planRevenueActions'])
        self.assertEqual('pro', snapshot['planRevenueActions'][0]['plan'])
        self.assertEqual(199, snapshot['planRevenueActions'][0]['customerGap'])
        self.assertEqual(5970, snapshot['planRevenueActions'][0]['remainingMrrToTargetUsd'])
    ...
    def test_next_best_action_flags_account_handoff_without_key_attempts(self):
        ...
        self.assertIn('/account', action['executionChecklist'][0]['action'])
        ...
        signed_out_action = router.launch_next_best_action(
            stages,
            rates,
            {'estimatedCurrentMrrUsd': 60, 'targetMrrUsd': 10000},
            {
                **activation_follow_ups,
                'keyRecoverySignedOutPrompts': 2,
                'keyRecoverySignedOutPromptsByState': {'same_email': 2},
            },
            [],
        )
        self.assertIn('still signed out', signed_out_action['action'])
        self.assertIn('same-email magic-link or same-OAuth recovery events', signed_out_action['action'])
        self.assertIn('Signed-out recovery prompts are now tracked', signed_out_action['executionChecklist'][1]['action'])
        self.assertIn('account_magic_link_requested/sent or same-OAuth telemetry', signed_out_action['executionChecklist'][1]['successMetric'])
    ...
    def test_operator_execution_packet_is_aggregate_only(self):
        ...
        self.assertFalse(readiness['managedResaleEnabled'])
        self.assertTrue(readiness['secretFree'])
        self.assertTrue(readiness['publicSafe'])
        self.assertFalse(readiness['sendsEmail'])
        self.assertFalse(readiness['mutatesRuntime'])
        ...
        self.assertTrue(all(row['managedResaleEnabled'] is False for row in readiness['nextActions']))
        self.assertTrue(all(row['secretFree'] is True for row in readiness['nextActions']))
        self.assertTrue(all(row['publicSafe'] is True for row in readiness['nextActions']))
        self.assertTrue(all(row['sendsEmail'] is False for row in readiness['nextActions']))
        self.assertTrue(all(row['mutatesRuntime'] is False for row in readiness['nextActions']))
        self.assertTrue(all(row['privacy']['containsProviderResponses'] is False for row in readiness['nextActions']))
        self.assertTrue(all(row['privacy']['aggregateOnly'] is True for row in readiness['nextActions']))
    ...
    def test_operator_execution_packet_marks_auth_repair_segments_review_only(self):
        ...
        self.assertFalse(readiness['sendsEmail'])
        self.assertFalse(readiness['mutatesRuntime'])
        self.assertEqual('approve_after_review', readiness['decisionLines'][0]['id'])
        self.assertIn('APPROVE_ACTIVATION_FOLLOWUP segment="verified"', readiness['decisionLines'][0]['value'])
        self.assertIn('HOLD_ACTIVATION_FOLLOWUP segment="verified"', readiness['decisionLines'][1]['value'])
        self.assertFalse(readiness['decisionLines'][0]['mutatesRuntime'])
        self.assertFalse(readiness['decisionLines'][0]['sendsEmail'])
    ...
    def test_launch_marketing_counts_group_anonymous_managed_access_interest(self):
        ...
        def fake_select(table, query, timeout=8):
            ...
            return [
                ...
                {
                    'event': 'managed_access_quick_form_started',
                    'plan': 'max',
                    'created_at': '2026-06-19T00:00:00Z',
                    'source_page': 'https://sagerouter.dev/managed-access',
                    'metadata': {
    ...
    def test_launch_marketing_funnel_counts_group_events_without_identity(self):
        ...
        def fake_select(table, query, timeout=8):
            ...
            return [
                ...
                {
                    'event': 'api_reference_key_recovery_clicked',
                    'plan': 'pro',
                    'created_at': '2026-06-19T00:00:00Z',
                    'metadata': {
                        'source': 'api-reference',
                        'state': 'api-reference-hero-recovery',
                    },
                },
                {
                    'event': 'api_troubleshooting_key_recovery_clicked',
                    'plan': 'pro',
                    'created_at': '2026-06-19T00:00:00Z',
                    'metadata': {
                        'source': 'api-troubleshooting',
                        'state': 'api-troubleshooting-hero-recovery',
                    },
                },
                {
                ...
                {
                    'event': 'account_setup_handoff_viewed',
                    'plan': 'pro',
                    'created_at': '2026-06-19T00:00:00Z',
                    'metadata': {
                        'source': 'account',
                        'state': 'login-key-recovery',
                    },
                },
                {
                ...
                {
                    'event': 'account_key_recovery_create_clicked',
                    'plan': 'pro',
                    'created_at': '2026-06-19T00:00:00Z',
                    'metadata': {
                        'source': 'account',
                        'state': 'key_recovery_manual',
                    },
                },
                {
                    'event': 'account_key_recovery_create_failed',
                    'plan': 'pro',
                    'created_at': '2026-06-19T00:00:00Z',
                    'metadata': {
                        'source': 'account',
                ...
                {
                    'event': 'setup_key_recovery_auto_account_redirected',
                    'plan': 'pro',
                    'created_at': '2026-06-19T00:00:00Z',
                    'metadata': {
                ...
                {
                    'event': 'setup_key_recovery_account_setup_scheduled',
                    'plan': 'pro',
                    'created_at': '2026-06-19T00:00:00Z',
                    'metadata': {
                        'source': 'setup-key-recovery',
                ...
                {
                    'event': 'setup_key_recovery_account_setup_paused',
                    'plan': 'pro',
                    'created_at': '2026-06-19T00:00:00Z',
                    'metadata': {
                        'source': 'setup-key-recovery',
                ...
                {
                    'event': 'outreach_email_draft_opened',
                    'plan': None,
                    'created_at': '2026-06-19T00:00:00Z',
                    'metadata': {
                        'source': 'founder-sales',
                        'snippet': 'founder-recommended-first-reply-email-draft',
                    },
                },
                {
        ...
        self.assertIsNone(error)
        self.assertEqual(71, metrics['total'])
        ...
        self.assertEqual(1, metrics['events']['api_reference_key_recovery_clicked'])
        ...
        self.assertEqual(1, metrics['events']['account_setup_handoff_viewed'])
        self.assertEqual(1, metrics['events']['account_key_recovery_signed_out_prompt_shown'])
        ...
        self.assertEqual(1, metrics['events']['setup_key_recovery_auto_account_redirected'])
        self.assertEqual(1, metrics['events']['setup_key_recovery_account_setup_scheduled'])
        self.assertEqual(1, metrics['events']['setup_key_recovery_account_setup_paused'])
        ...
        self.assertEqual(1, metrics['events']['outreach_email_draft_opened'])
        ...
        self.assertEqual(1, metrics['events']['status_managed_provider_authorization_review_copied'])
        ...
        self.assertEqual(2, metrics['founderSalesOutreachCopies'])
        ...
        self.assertEqual(1, metrics['providerAuthorizationReviewCopiesBySnippet']['operator-provider-authorization-review-packet'])
        ...
        self.assertEqual(12, metrics['keyFirstRedirects'])
        self.assertEqual(1, metrics['keyRecoveryHandoffScheduled'])
        self.assertEqual(1, metrics['keyRecoveryHandoffScheduledByState']['recovery_auto'])
        self.assertEqual(1, metrics['keyRecoveryHandoffPaused'])
        self.assertEqual(1, metrics['keyRecoveryHandoffPausedByState']['recovery_auto'])
        ...
        self.assertEqual(1, metrics['keyFirstRedirectsByState']['operator_activation'])
        ...
        self.assertEqual(1, metrics['keyFirstRedirectsByState']['login-key-recovery'])
        self.assertEqual(17, metrics['keyRecoveryViews'])
        self.assertEqual(1, metrics['keyRecoveryViewsByState']['github'])
        self.assertEqual(1, metrics['keyRecoveryViewsByState']['login-key-recovery'])
        self.assertEqual(1, metrics['keyRecoverySignedOutPrompts'])
        self.assertEqual(1, metrics['keyRecoverySignedOutPromptsByState']['same_email'])
        self.assertEqual(2, metrics['keyCreateAttempts'])
        self.assertEqual(1, metrics['keyCreateAttemptsByState']['saved_key_recovery_auto_key'])
        self.assertEqual(1, metrics['keyCreateAttemptsByState']['key_recovery_manual'])
        self.assertEqual(1, metrics['keyCreateSuccesses'])
        self.assertEqual(1, metrics['keyCreateSuccessesByState']['created'])
        self.assertEqual(1, metrics['keyCreateFailures'])
        self.assertEqual(1, metrics['keyCreateFailuresByState']['rate_limited'])
        ...
        self.assertEqual(1, metrics['keyRecoveryViewsByState']['operator_activation'])
        ...
        self.assertEqual(1, metrics['keyRecoveryViewsByState']['api-reference-hero-recovery'])
        self.assertEqual(1, metrics['keyRecoveryViewsByState']['api-troubleshooting-hero-recovery'])
        ...
        self.assertEqual(13, metrics['sourceSurfaces']['account'])
        ...
        self.assertEqual(3, metrics['sourceSurfaces']['setup-key-recovery'])
        self.assertEqual(5, metrics['sourceSurfaces']['other'])
        ...
        self.assertEqual(60, metrics['attributionChannels']['direct'])
        ...
        self.assertTrue(all(row['secretFree'] and row['publicSafe'] for row in actions))
        self.assertTrue(all(row['sendsEmail'] is False and row['mutatesRuntime'] is False for row in actions))
        self.assertTrue(all(row['managedResaleEnabled'] is False for row in actions))
        self.assertTrue(all(row['ctaPath'].startswith(('https://sagerouter.dev/', 'https://app.sagerouter.dev/')) for row in actions))
        self.assertIn('utm_campaign=', json.dumps(actions))
        self.assertFalse(any(row['privacy']['containsApiKeys'] for row in actions))
    ...
    def test_origin_model_listing_requires_paid_or_operator_auth_when_enabled(self):
        ...
        self.assertEqual('https://sagerouter.dev/setup-key-recovery?utm_source=api-auth&utm_medium=recovery&utm_campaign=signup_to_key_recovery', anonymous.payload['keyRecoveryUrl'])
        ...
        self.assertIn('OPENAI_BASE_URL=https://api.sagerouter.dev/v1', anonymous.payload['setupSnippet'])
        self.assertIn('sk_sage_REPLACE_WITH_GENERATED_KEY', anonymous.payload['setupSnippet'])
        self.assertIn('$OPENAI_BASE_URL/models', anonymous.payload['setupSnippet'])
        self.assertEqual('~/.codex/config.toml', anonymous.payload['codexProfile']['configPath'])
        self.assertEqual('sage-router-hosted', anonymous.payload['codexProfile']['providerName'])
        self.assertEqual('sage-router-frontier', anonymous.payload['codexProfile']['profileName'])
        self.assertEqual('https://api.sagerouter.dev/v1/', anonymous.payload['codexProfile']['baseUrl'])
        self.assertEqual('OPENAI_API_KEY', anonymous.payload['codexProfile']['envKey'])
        self.assertEqual('responses', anonymous.payload['codexProfile']['wireApi'])
        self.assertEqual('sage-router/frontier', anonymous.payload['codexProfile']['model'])
        self.assertEqual('codex --profile sage-router-frontier', anonymous.payload['codexProfile']['runCommand'])
        self.assertIn('[model_providers.sage-router-hosted]', anonymous.payload['codexSetupSnippet'])
        self.assertIn('[profiles.sage-router-frontier]', anonymous.payload['codexSetupSnippet'])
        self.assertIn('base_url = "https://api.sagerouter.dev/v1/"', anonymous.payload['codexSetupSnippet'])
        self.assertIn('env_key = "OPENAI_API_KEY"', anonymous.payload['codexSetupSnippet'])
        self.assertIn('wire_api = "responses"', anonymous.payload['codexSetupSnippet'])
        self.assertIn('sk_sage_REPLACE_WITH_GENERATED_KEY', anonymous.payload['codexSetupSnippet'])
        self.assertIn('codex --profile sage-router-frontier', anonymous.payload['codexSetupSnippet'])
    ...
    def test_origin_model_post_auth_error_includes_onboarding_guidance(self):
        ...
        self.assertEqual('https://sagerouter.dev/setup-key-recovery?utm_source=api-auth&utm_medium=recovery&utm_campaign=signup_to_key_recovery', handler.payload['keyRecoveryUrl'])
        ...
        self.assertIn('OPENAI_BASE_URL=https://api.sagerouter.dev/v1', handler.payload['setupSnippet'])
        self.assertIn('sk_sage_REPLACE_WITH_GENERATED_KEY', handler.payload['setupSnippet'])
        self.assertEqual('sage-router-frontier', handler.payload['codexProfile']['profileName'])
        self.assertEqual('sage-router/frontier', handler.payload['codexProfile']['model'])
        self.assertIn('[model_providers.sage-router-hosted]', handler.payload['codexSetupSnippet'])
        self.assertIn('codex --profile sage-router-frontier', handler.payload['codexSetupSnippet'])
