...
class TailnetEdgeAuthTests(unittest.TestCase):
    ...
    def test_model_api_auth_errors_include_onboarding_links(self):
        ...
        self.assertIn("OPENAI_BASE_URL=https://api.sagerouter.dev/v1", payload["setupSnippet"])
        self.assertIn("sk_sage_REPLACE_WITH_GENERATED_KEY", payload["setupSnippet"])
        self.assertIn('$OPENAI_BASE_URL/models', payload["setupSnippet"])
        self.assertEqual("~/.codex/config.toml", payload["codexProfile"]["configPath"])
        self.assertEqual("sage-router-hosted", payload["codexProfile"]["providerName"])
        self.assertEqual("sage-router-frontier", payload["codexProfile"]["profileName"])
        self.assertEqual("https://api.sagerouter.dev/v1/", payload["codexProfile"]["baseUrl"])
        self.assertEqual("OPENAI_API_KEY", payload["codexProfile"]["envKey"])
        self.assertEqual("responses", payload["codexProfile"]["wireApi"])
        self.assertEqual("sage-router/frontier", payload["codexProfile"]["model"])
        self.assertEqual("codex --profile sage-router-frontier", payload["codexProfile"]["runCommand"])
        self.assertIn("[model_providers.sage-router-hosted]", payload["codexSetupSnippet"])
        self.assertIn("[profiles.sage-router-frontier]", payload["codexSetupSnippet"])
        self.assertIn('base_url = "https://api.sagerouter.dev/v1/"', payload["codexSetupSnippet"])
        self.assertIn('env_key = "OPENAI_API_KEY"', payload["codexSetupSnippet"])
        self.assertIn('wire_api = "responses"', payload["codexSetupSnippet"])
        self.assertIn("sk_sage_REPLACE_WITH_GENERATED_KEY", payload["codexSetupSnippet"])
        self.assertIn("codex --profile sage-router-frontier", payload["codexSetupSnippet"])
