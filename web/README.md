...
- Pricing uses the same copy-first activation CTA as quickstart: `Copy 60-second setup bundle`, `pricing-full-setup-bundle`, `quickstart_snippet_copied` telemetry, and a revealed `Create Pro key next` handoff for buyer-intent setup demand.
...
- Hosted account page primary activation creates the first `sk_sage_*` key directly for signed-in, verified users without one, preserving the one-time key display and saved checkout intent.
- Dedicated API quickstart page at `/quickstart` with a copy-first `Copy 60-second setup bundle` action, a post-copy `Create Pro key next` handoff, hosted `sk_sage_*` key setup, `OPENAI_BASE_URL=https://api.sagerouter.dev/v1`, `sage-router/frontier`, curl, JavaScript, Python, Codex, and error troubleshooting.
