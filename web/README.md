...
- Login key recovery at `/login.html?start=create_key` keeps same-email recovery first, shows the generated-key-before-checkout steps, embeds an inline same-email setup-link form in the recovery panel, preserves the account activation URL, and records `login_key_recovery_magic_link_*` plus `login_key_recovery_account_setup_clicked` so the launch funnel can separate passive no-key recovery views from setup-link requests and click-throughs toward API-key setup.
