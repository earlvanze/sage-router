...
- Pricing uses the same copy-first activation CTA as quickstart: `Copy 60-second setup bundle`, `pricing-full-setup-bundle`, and `quickstart_snippet_copied` telemetry for buyer-intent setup demand.
...
- Dedicated API quickstart page at `/quickstart` with a copy-first `Copy 60-second setup bundle` action, hosted `sk_sage_*` key setup, `OPENAI_BASE_URL=https://api.sagerouter.dev/v1`, `sage-router/frontier`, curl, JavaScript, Python, Codex, and error troubleshooting.
