...
- Pricing uses the same copy-first activation CTA as quickstart: `Copy 60-second setup bundle`, `pricing-full-setup-bundle`, `quickstart_snippet_copied` telemetry, `start=checkout` plan handoffs, and a key-first proof block that tells buyers to create the generated `sk_sage_*` key before checkout unlocks hosted routing.
