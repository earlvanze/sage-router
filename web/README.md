...
- Login key recovery at `/login.html?start=create_key` keeps same-email recovery first, shows the generated-key-before-checkout steps, embeds an inline same-email setup-link form in the recovery panel, exposes a static fully attributed `Create API key next` handoff with `setup=login-key-recovery`, `source_surface=recovery`, and `next=generated-key`, preserves the account activation URL, and records `login_key_recovery_magic_link_*` plus `login_key_recovery_account_setup_clicked` so the launch funnel can separate passive no-key recovery views from setup-link requests and click-throughs toward API-key setup.
- Account and analytics auth panels expose same-email `Finish setup key` recovery beside account/API-access CTAs, recording `account_auth_key_recovery_clicked` and `analytics_key_recovery_clicked` for returning no-key users already on app surfaces.
- Model-gateway and OpenRouter comparison pages expose GitHub/email setup plus `Finish setup key` recovery, recording `gateway_compare_key_recovery_clicked` for returning no-key visitors without changing the OpenRouter BYOK-vs-managed-resale boundary.
- Dedicated API quickstart page at `/quickstart` with `Copy 60-second setup bundle`, an always-visible `Create API key next` handoff with `quickstart_setup_next_clicked` telemetry, `Finish setup key` no-key recovery with `quickstart_key_recovery_clicked` telemetry, hosted `sk_sage_*` key setup, `OPENAI_BASE_URL=https://api.sagerouter.dev/v1`, `sage-router/frontier`, curl, JavaScript, Python, Codex, and error troubleshooting. Public marketing, docs, status, support, pricing, billing, comparison, and article recovery CTAs route through `/setup-key-recovery` before the same-email app login handoff. The shared email activation helper also inserts `Finish setup key` recovery with `shared_email_key_recovery_clicked` telemetry on pages that use `data-email-activation-form`; the shared article activation dock injects hero `Finish setup key` recovery beside Pro create-key buttons on article and provider pages with `content_article_key_recovery_clicked` telemetry, and its inline/sticky `Copy setup first` CTAs record `content_article_snippet_copied` with `article-inline-setup-bundle` or `article-sticky-setup-bundle` before switching to `Create API key next`.
...
- Dedicated setup-key recovery page at `/setup-key-recovery` for API 401 `keyRecoveryUrl` handoffs and returning no-key signups. It makes the same-email setup link the first action so recovery traffic can produce `setup_key_recovery_magic_link_requested` before another passive account view, while the signed-in account setup link, app login URL, and direct GitHub OAuth recovery remain available for the same identity. API-auth recovery links (`utm_source=api-auth` plus `signup_to_key_recovery`) auto-open generated-key account setup with attribution preserved. It keeps generated-key-before-checkout account setup, includes a placeholder setup bundle, and records `setup_key_recovery_*` events plus setup-copy and managed-access review telemetry without collecting prompts, credentials, or generated keys. The hosted login recovery panel mirrors that flow with a copy-first placeholder setup button and same-GitHub recovery action measured by `login_key_recovery_setup_copied` and `login_key_recovery_oauth_clicked`.
...
- Founder sales kit at `/founder-sales-kit` with first-viewport `Copy next Pro reply` and `Copy 60-second setup bundle` actions plus copyable Pro activation, Max implementation, gateway migration, one-subscription review, and calculator follow-up outreach. The hero setup copy records `quickstart_snippet_copied` with `founder-hero-setup-bundle`, outreach copies record `outreach_snippet_copied`, links use measured `utm_source=founder-sales`, and the page keeps no-secret boundaries, sitemap, LLM discovery, and readiness checks. The private launch-funnel dashboard also renders a copyable next outreach snippet from aggregate MRR gap data and records `outreach_snippet_copied` so founder-sales work is visible before broad automation.
