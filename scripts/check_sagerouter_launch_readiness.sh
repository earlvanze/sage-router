  ...
  case "${MANAGED_PROVIDER_RESALE_ENABLED,,}" in
    ...
    *)
      ...
      if [[ "$enabled" == "false" && "$safe_disabled_status" == "true" && "$readiness_satisfied" == "false" && "$terms_ack" == "false" && "$controls_ok" == "true" && "$cost_controls_ok" == "true" && "$unit_economics_ok" == "true" && "$unit_economics_safe_thresholds_ok" == "true" && "$provider_family_boundary_ok" == "true" && "$one_subscription_action_ok" == "true" && "$missing_controls_explain_blockers_ok" == "true" && "$readiness_setup_ok" == "true" && "$onboarding_sequence_ok" == "true" && "$margin_percent" =~ ^[0-9]+$ && "$margin_percent" -ge 30 && "$missing_count" =~ ^[0-9]+$ && "$missing_count" -gt 0 ]]; then
        pass "managed provider access remains disabled while reviewRequested=${requested}; provider resale terms, provider authorization evidence, provider allowlist, positive unit economics, provider-family BYOK boundary, margin policy, quotas, operator audit events, setup guard, onboarding sequence, and acceptable-use controls are still enforced"
      else
