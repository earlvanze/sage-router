...
check_public_auth_gate() {
  ...
  key_recovery_url="$(jq -r '.keyRecoveryUrl // empty' /tmp/sage-router-readiness-body 2>/dev/null || true)"
...
check_managed_provider_access_guard() {
  ...
  ' /tmp/sage-router-readiness-body)"
  onboarding_sequence_ok="$(jq -r '
    (.publicLaunch.managedProviderAccess.onboardingSequence // []) as $steps |
    ($steps | length) >= 4 and
    ($steps | map(.id) | index("collect_provider_authorization")) and
    ($steps | map(.id) | index("review_terms_and_public_controls")) and
    ($steps | map(.id) | index("verify_private_unit_economics")) and
    ($steps | map(.id) | index("final_private_beta_enablement_review")) and
    ($steps | all(.publicSafe == true and .secretFree == true)) and
    ($steps | all((.privacy.containsSecrets // false) == false)) and
    ($steps | all((.privacy.containsProviderCredentials // false) == false)) and
  ...
  case "${MANAGED_PROVIDER_RESALE_ENABLED,,}" in
    1|true|yes|on)
      if [[ "$enabled" == "true" &&
            ...
            "$onboarding_sequence_ok" == "true" &&
            "$margin_percent" =~ ^[0-9]+$ &&
            "$margin_percent" -ge 30 &&
            "$missing_count" == "0" ]]; then
        pass "managed provider access is explicitly enabled with acknowledged resale terms, provider authorization evidence, a provider allowlist, positive unit economics, provider-family BYOK boundary, margin policy, quotas, operator audit events, onboarding sequence, and acceptable-use controls"
      else
        fail "managed provider access enabled without complete controls, including acknowledged resale terms, provider authorization evidence, provider allowlist, positive unit economics, provider-family BYOK boundary, one-subscription action link, operator audit events, managed-access setup guard, onboarding sequence, and managed-access acceptable-use boundary: enabled=${enabled} requested=${requested:-missing} readinessSatisfied=${readiness_satisfied:-missing} status=${status:-missing} terms=${terms_url:+present} termsAcknowledged=${terms_ack:-missing} authorizationEvidence=${auth_evidence:-missing} allowedProviderFamilies=${allowlist_count:-missing} margin=${margin_url:+present} minimumGrossMarginPercent=${margin_percent:-missing} positiveUnitEconomics=${unit_economics_ok:-missing} costModelConfigured=${cost_model_configured:-missing} unitEconomicsSatisfied=${unit_economics_satisfied:-missing} unitEconomicsPlans=${unit_economics_plans_ok:-missing} unitEconomicsSafeThresholds=${unit_economics_safe_thresholds_ok:-missing} providerFamilyBoundary=${provider_family_boundary_ok:-missing} oneSubscriptionAction=${one_subscription_action_ok:-missing} missingControlsExplainBlockers=${missing_controls_explain_blockers_ok:-missing} readinessSetup=${readiness_setup_ok:-missing} onboardingSequence=${onboarding_sequence_ok:-missing} acceptableUse=${acceptable_url:-missing} controls=${controls_ok:-missing} costControls=${cost_controls_ok:-missing} missingControls=${missing_count:-missing}"
      fi
      ;;
    *)
      local safe_disabled_status
      if [[ ("$requested" == "false" && "$status" == "disabled_pending_provider_terms") ||
            ("$requested" == "true" && "$status" == "requires_readiness_verification") ]]; then
        safe_disabled_status="true"
      else
        safe_disabled_status="false"
      fi
      if [[ "$enabled" == "false" && "$safe_disabled_status" == "true" && "$readiness_satisfied" == "false" && "$terms_ack" == "false" && "$controls_ok" == "true" && "$cost_controls_ok" == "true" && "$unit_economics_ok" == "true" && "$unit_economics_safe_thresholds_ok" == "true" && "$provider_family_boundary_ok" == "true" && "$one_subscription_action_ok" == "true" && "$missing_controls_explain_blockers_ok" == "true" && "$readiness_setup_ok" == "true" && "$onboarding_sequence_ok" == "true" && "$margin_percent" =~ ^[0-9]+$ && "$margin_percent" -ge 30 && "$missing_count" =~ ^[0-9]+$ && "$missing_count" -gt 0 ]]; then
        pass "managed provider access remains disabled until provider resale terms, provider authorization evidence, provider allowlist, positive unit economics, provider-family BYOK boundary, margin policy, quotas, operator audit events, setup guard, onboarding sequence, and acceptable-use controls are enforced; reviewRequested=${requested}"
      else
        fail "managed provider access guard unexpected: enabled=${enabled} requested=${requested:-missing} readinessSatisfied=${readiness_satisfied:-missing} status=${status:-missing} termsAcknowledged=${terms_ack:-missing} authorizationEvidence=${auth_evidence:-missing} allowedProviderFamilies=${allowlist_count:-missing} minimumGrossMarginPercent=${margin_percent:-missing} positiveUnitEconomics=${unit_economics_ok:-missing} unitEconomicsSafeThresholds=${unit_economics_safe_thresholds_ok:-missing} providerFamilyBoundary=${provider_family_boundary_ok:-missing} oneSubscriptionAction=${one_subscription_action_ok:-missing} missingControlsExplainBlockers=${missing_controls_explain_blockers_ok:-missing} readinessSetup=${readiness_setup_ok:-missing} onboardingSequence=${onboarding_sequence_ok:-missing} controls=${controls_ok:-missing} costControls=${cost_controls_ok:-missing} missingControls=${missing_count:-missing}, expected safe disabled status disabled_pending_provider_terms or requires_readiness_verification"
      fi
      ;;
  esac
}
...
check_hosted_onboarding_pages() {
  ...
  if [[ "$login_code" == "200" ]] && ! grep -q "Create API key next" /tmp/sage-router-readiness-body; then
    login_code="200:missing-login-key-recovery-key-next"
  fi
  ...
  if [[ "$launch_funnel_js_code" == "200" ]] && ! grep -q "data-founder-sales-promotion=\"managed-access-gap\"" /tmp/sage-router-readiness-body; then
    launch_funnel_js_code="200:missing-managed-access-contact-promotion-founder-actions"
  fi
  ...
  if [[ "$status_code" == "200" ]] && ! grep -q "status-copy-activation-worksheet-refresh" /tmp/sage-router-readiness-body; then
    status_code="200:missing-status-activation-worksheet-refresh-copy"
  fi
  ...
  if [[ "$status_js_code" == "200" ]] && ! grep -q "snippet: 'status-activation-worksheet-refresh'" /tmp/sage-router-readiness-body; then
    status_js_code="200:missing-status-activation-worksheet-refresh-snippet"
  fi
...
check_funnel_event_endpoint() {
  ...
  allowed_events="$(jq -r '
    ...
    ((.allowedEvents // []) | index("account_key_recovery_account_load_retry_scheduled") != null) and
    ((.allowedEvents // []) | index("account_key_recovery_key_list_load_failed") != null) and
...
check_marketing_pricing_page() {
  ...
  if [[ "$page_code" == "200" ]] && ! grep -q "pricing-copy-managed-access-review-packet" /tmp/sage-router-readiness-body; then
    page_code="200:missing-pricing-managed-access-review-packet-copy"
  fi
  if [[ "$page_code" == "200" ]] && ! grep -q "pricing-one-subscription-review-packet" /tmp/sage-router-readiness-body; then
    page_code="200:missing-pricing-managed-access-review-packet-snippet"
  fi
...
check_marketing_founder_sales_kit_page() {
  ...
  if [[ "$page_code" == "200" ]] && ! grep -q "outreach_email_draft_opened" /tmp/sage-router-readiness-body; then
    page_code="200:missing-founder-sales-email-draft-funnel"
  fi
...
check_marketing_managed_access_page() {
  ...
  if [[ "$page_code" == "200" ]] && ! grep -q "recordManagedAccessContactHandoff(state, '#managed-access-first-screen-contact-choice', 'managed-access-first-screen-contact-choice')" /tmp/sage-router-readiness-body; then
...
check_admin_token() {
  ...
  funnel_ok="$(jq -r '
    ...
    ((.managedAccessDemand.intent // {}) | has("max-implementation") and has("private-deployment") and has("gateway-migration") and has("one-subscription") and has("ollama") and has("openai") and has("anthropic") and has("unknown")) and
	    ...
	    ((.managedAccessDropoff // {}) | has("status") and has("priority") and has("counts") and has("rates") and (.managedResaleEnabled == false) and (.privacy.containsEmails == false)) and
	    ((.managedAccessDropoff.counts // {}) | has("preContactSignals") and has("fastPathEngagements") and has("quickRequestsSubmitted") and has("quickRequestsReceived") and has("contactableRequests")) and
