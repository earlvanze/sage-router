...
if [[ "$MANAGED_ACCESS_DROPOFF_PACKET" == "1" ]]; then
  jq -r --argjson days "$DAYS" '
    ...
    | ($events.managed_access_contact_handoff_prompted // 0) as $handoff_prompts
    ...
    | (
        ...
        elif ($handoff_prompts > 0 and ($focused + $started + $drafts + $contact_packets + $submitted + $received) <= 0) then
          "The idle contact handoff is visible, but no copy, draft, focus, or submit action followed; tighten the prompt copy and route warm outreach to a direct contact action."
        elif ($presented > 0 and $landed <= 0 and $handoff_prompts <= 0 and ($focused + $started + $drafts + $contact_packets + $submitted + $received) <= 0) then
          "The fast form is visible, but no contact-capture landing or idle handoff prompt is recorded; verify the managed-access page script and route operator/founder-sales CTAs to the anchored fast form."
    ...
    | [
        ...
        "Fast funnel: quickPresented=\(n($presented)); contactCaptureLanded=\(n($landed)); handoffPrompts=\(n($handoff_prompts)); quickFocused=\(n($focused)); contactPackets=\(n($contact_packets)); emailDrafts=\(n($drafts)); quickStarted=\(n($started)); quickValidationFailed=\(n($validation_failed)); quickSubmitted=\(n($submitted)); quickReceived=\(n($received)).",
        ...
        "- If quickPresented rises but contactCaptureLanded and handoffPrompts stay at zero, verify the managed-access page script and use the anchored operator CTA from this packet in founder-sales and support replies.",
        "- If handoffPrompts rises but quickFocused, contactPackets, and emailDrafts stay at zero, make the sticky handoff copy more direct and use the copy/email actions in warm outreach.",
        ...
        "",
        "Copy-ready contact handoff:",
        "If one subscription is the buying trigger, use the private review path instead of assuming public bundled resale is enabled.",
        "Review link: \($conversion.ctaPath // "https://sagerouter.dev/managed-access?intent=one-subscription#managed-access-quick-form")",
        "Immediate fallback: Max works today with authorized BYOK/provider access at https://app.sagerouter.dev/account.html?plan=max&start=create_key&utm_source=operator&utm_medium=managed-access-dropoff&utm_campaign=sage-router-launch",
        "Boundary: Sage Router will not enable managed OpenAI, Anthropic, Ollama, or other provider access until provider authorization, terms acknowledgment, allowed-provider controls, private cost model, positive unit economics, quotas, audit events, abuse controls, and acceptable-use review pass.",
        "Do not paste prompts, provider credentials, OAuth tokens, generated API keys, private keys, customer data, raw provider responses, provider agreements, or private provider costs into the request.",
        "",
...
if [[ "$FOUNDER_SALES_PACKET" == "1" || "$RECORD_FOUNDER_SALES" == "1" ]]; then
  jq -r '
    ...
    | [
        ...
        "https://sagerouter.dev/managed-access?intent=max-implementation&utm_source=founder-sales&utm_medium=direct&utm_campaign=sage-router-launch&utm_content=operator-max-review#managed-access-quick-form",
        "",
        "One-subscription review without assuming public bundled resale is enabled:",
        "https://sagerouter.dev/managed-access?intent=one-subscription&utm_source=founder-sales&utm_medium=direct&utm_campaign=sage-router-launch&utm_content=operator-one-subscription-review#managed-access-quick-form",
        "",
...
if [[ "$RAW_JSON" == "1" ]]; then
  ...
  def managed_access_dropoff($root):
    ...
    | {
        ...
        contactHandoffPrompts: ($events.managed_access_contact_handoff_prompted // 0),
    ...
    | (
        ...
        elif (($counts.contactHandoffPrompts // 0) > 0) then {
    ...
    );
  {
    ...
    marketingIntent: (
      ...
      | {
        ...
        cloudflareBicTokenScopeCopiesBySnippet: ($marketing.cloudflareBicTokenScopeCopiesBySnippet // {}),
    ...
    managedProviderReadiness: (
      (
        ...
        // {
          ...
          readinessSetup: {
            ...
            providerSourceReviewCommand: (.pricing.publicLaunch.managedProviderAccess.readinessSetup.providerSourceReviewCommand // "scripts/configure_managed_provider_resale_readiness.sh --provider-source-review-packet"),
      ...
      | .readinessSetup = ((.readinessSetup // {}) + {
          ...
          providerSourceReviewCommand: (if ((.readinessSetup.providerSourceReviewCommand // "") != "") then .readinessSetup.providerSourceReviewCommand else "scripts/configure_managed_provider_resale_readiness.sh --provider-source-review-packet" end),
...
jq -r --arg days "$DAYS" --slurpfile recoveryProof "$recovery_tmp" '
  ...
  | [
      ...
      "- Activation approval decision copies: \(n($marketing.activationApprovalDecisionCopies))",
      "- Activation approval decision snippets: \(buckets($marketing.activationApprovalDecisionCopiesBySnippet))",
      "- Cloudflare BIC token-scope copies: \(n($marketing.cloudflareBicTokenScopeCopies))",
      "- Cloudflare BIC token-scope snippets: \(buckets($marketing.cloudflareBicTokenScopeCopiesBySnippet))",
      ...
      "",
      "- Status: \($managedConversion.status // "unknown"); priority=\($managedConversion.priority // "monitor"); leadGap=\(n($managedConversion.contactableLeadGap)); managedResaleEnabled=false.",
      "- Drop-off packet: scripts/summarize_sagerouter_launch_funnel.sh --days \($days) --managed-access-dropoff-packet",
      "- Contactable CTA: \($managedConversion.ctaPath // "https://sagerouter.dev/managed-access?intent=one-subscription#managed-access-quick-form")",
      ...
      "- Success metric: \($managedConversion.successMetric // "managedAccessBetaInterest or managed_access_quick_request_received increases without enabling managed provider resale.")",
      ...
      "- Terminal packet: scripts/summarize_sagerouter_launch_funnel.sh --days \($days) --founder-sales-packet",
      ...
      "- Recording command: scripts/summarize_sagerouter_launch_funnel.sh --days \($days) --record-founder-sales",
      ...
      "- Provider source review packet: \(if (($managed.readinessSetup.providerSourceReviewCommand // "") != "") then $managed.readinessSetup.providerSourceReviewCommand else "scripts/configure_managed_provider_resale_readiness.sh --provider-source-review-packet" end)",
