...
- `https://sagerouter.dev/quickstart` is the hosted API first-request path. It leads with a copy-first `Copy 60-second setup bundle` action, reveals a `Create Pro key next` account handoff after copy, then shows `OPENAI_BASE_URL=https://api.sagerouter.dev/v1`, generated `sk_sage_*` key setup, the `sage-router/frontier` profile, premium `sage-router/fusion`, curl, JavaScript, Python, and Codex examples, plus 401/402/429/503 troubleshooting.
- `https://sagerouter.dev/pricing` uses the same copy-first activation pattern for buyer-intent traffic: `Copy 60-second setup bundle` records `quickstart_snippet_copied` with `pricing-full-setup-bundle`, plan links preserve `start=checkout`, and the key-first proof block makes the Pro path create a generated `sk_sage_*` key before checkout.
