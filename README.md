...
- Shared marketing email activation forms insert `Copy setup first`, optional GitHub Pro OAuth, and `Finish setup key` recovery with `shared_email_key_recovery_clicked` telemetry so calculator, Fusion, integrations, API docs, and migration pages all feed the same generated-key recovery path.
