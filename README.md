...
- `https://sagerouter.dev` keeps a persistent bottom activation bar with `Create API key`, `Copy 60-second setup`, and `Finish setup key` so landing-page visitors can copy the hosted setup bundle, create the generated key, or recover a no-key signup after scrolling without joining a waitlist.
