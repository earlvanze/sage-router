...
- `https://app.sagerouter.dev/launch-funnel.html` also renders a `Copy first-request setup` action in the private Do Next dock. When `nextBestAction.evidence.nonGatedSetupCopyFallback` is true, that action is promoted to the first primary dock control. It copies a placeholder `sk_sage_your_key_here` setup bundle, emits `status_first_request_setup_copied` with `operator-first-request-setup`, and lets the operator move the setup-copy activation KPI without sending activation email, exposing a real key, or changing billing/provider-resale state.
