...
- Anonymous model API 401 responses include a measured `keyRecoveryUrl` (`utm_source=api-auth`) in addition to `accountUrl`, `pricingUrl`, `statusUrl`, `openaiBaseUrl`, `apiKeyPrefix`, and a placeholder `setupSnippet`, so API-client failures can route returning no-key users into setup-key recovery, auto-open account setup for generated-key creation, and give CLI users a safe copyable next command.
