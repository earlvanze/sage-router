...
- `https://app.sagerouter.dev/status` is the public reliability and launch-actions page. It reads `/edge/health` and `/pricing` from the public API edge to show selected upstream ID, Tailnet/cloud backend class, CDN-style reliability evidence, lowest-latency retry failover policy, control-plane health, auth mode, rate-limit/quota enforcement, pre-auth generated-key attempt throttling, generated-key revocation posture, customer endpoint, plan limits, secret-free billing readiness, a first-screen no-secret founder-sales Pro reply, and copyable no-secret operator actions for activation email preflight, signup-to-key approval packet generation, founder-sales fallback, managed-provider resale dry-run/staging, first-request setup, and Cloudflare BIC verification without exposing customer data, internal upstream URLs, Tailnet hostnames, Stripe price IDs, provider credentials, private provider costs, or secrets. Status-page founder-sales copies emit `outreach_snippet_copied`; first-request setup copies emit `status_first_request_setup_copied` and count toward the setup-copy activation KPI.
