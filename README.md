...
- `https://sagerouter.dev` keeps a persistent bottom activation bar with `Create API key`, `60-second setup`, and `Finish setup key` so landing-page visitors and returning no-key signups can reach the generated-key recovery flow after scrolling without joining a waitlist.
