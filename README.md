...
- `https://app.sagerouter.dev/status` is the public reliability page. It reads `/edge/health` and `/pricing` from the public API edge to show selected upstream, Tailnet/cloud backend health, auth mode, customer endpoint, and plan limits without exposing customer data or secrets.
...
For acquisition and onboarding, `https://sagerouter.dev/model-routing-calculator`
helps prospects estimate routing savings, escalation rules, fallback gaps, and
review rates for one workflow before they create a hosted API key.
