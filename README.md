...
- The private launch funnel renders copyable `APPROVE_ACTIVATION_FOLLOWUP` and `HOLD_ACTIVATION_FOLLOWUP` decision records beside the no-secret approval packet, so the operator approval step is auditable without sending email, mutating queued follow-ups, exposing customer data, or bypassing the typed `SEND_ACTIVATION_FOLLOWUPS` gate.
