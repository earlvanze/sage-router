...
- `https://sagerouter.dev/pricing` uses the same measurable activation pattern for buyer-intent traffic: `Copy 60-second setup bundle` records `quickstart_snippet_copied` with `pricing-full-setup-bundle`, Pro activation links use `start=create_key`, `Create API key next` stays visible before and after copy, and the first viewport now includes a lowest-friction Lite path that sends $6 buyers to `start=create_key` before checkout. Lite/Max checkout links preserve `start=checkout`, adjacent Lite/Max key-first links use `start=create_key` and `pricing_setup_next_clicked`, and the key-first proof block makes the buyer create a generated `sk_sage_*` key before checkout. The page also includes a one-field managed-access review request that posts only contact plus allowlisted waitlist qualification buckets and records `managed_access_quick_request_*` telemetry; it captures one-subscription or Max/BYOK demand without enabling managed provider resale before provider authorization, terms, cost model, and margin controls pass.
