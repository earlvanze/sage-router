...
- Managed-access page visits now schedule a bounded idle focus on the fast review work-email field when the visitor has not interacted, recording the existing `managed_access_quick_form_focused` event with `managed-access-default-idle-focus` so anonymous one-subscription demand can move from form presentation to a contactable review path without sending email or enabling managed resale.
