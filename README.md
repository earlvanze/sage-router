...
[![ClawHub](https://img.shields.io/badge/ClawHub-v4.157.10-blue)](https://clawhub.ai/earlvanze/sage-router)
...
- `https://app.sagerouter.dev/status` is the public reliability and launch-actions page. It reads `/edge/health` and `/pricing` from the public API edge to show selected upstream ID, Tailnet/cloud backend class, CDN-style reliability evidence, lowest-latency retry failover policy, control-plane health, auth mode, rate-limit/quota enforcement, pre-auth generated-key attempt throttling, generated-key revocation posture, customer endpoint, plan limits, secret-free billing readiness, a first-screen no-secret founder-sales recommended first reply plus Pro reply, a first-screen hosted setup bundle, a first-screen no-secret one-subscription review packet and managed-access contact request, a first-screen no-secret signup-to-key activation approval packet with worksheet-refresh and post-review recording commands, and copyable no-secret operator actions for activation email preflight, signup-to-key approval packet generation, founder-sales fallback, managed-provider resale dry-run/staging, managed-provider terminal review recording, first-request setup, and Cloudflare BIC verification without exposing customer data, internal upstream URLs, Tailnet hostnames, Stripe price IDs, provider credentials, private provider costs, or secrets. Status-page founder-sales copies emit `outreach_snippet_copied`; status-page setup copies emit `status_first_request_setup_copied` and count toward the setup-copy activation KPI; status-page one-subscription packet copies emit `managed_access_review_packet_copied` with `status-one-subscription-review-packet`; status-page managed-access contact copies and drafts emit `managed_access_contact_packet_copied` / `managed_access_contact_draft_opened` with `status-managed-access-contact-*` snippets; status-page activation approval packet/review copies emit `status_activation_approval_packet_copied` with snippet identifiers such as `status-activation-approval-packet`, `status-activation-worksheet-refresh`, and `status-activation-review-record`; approve/hold decision copies are intentionally private-funnel only because the public status page cannot verify live approval-packet coverage; managed-provider record actions emit `status_managed_provider_outreach_copied`, `status_managed_provider_authorization_review_copied`, or `status_managed_provider_terms_review_copied` without acknowledging terms, staging evidence, writing costs, deploying, or enabling resale.
- `https://app.sagerouter.dev/launch-funnel.html` also renders a `Copy first-request setup` action in the private Do Next dock. When `nextBestAction.evidence.nonGatedSetupCopyFallback` is true, that action is also promoted into the first loaded dashboard panel and to the first primary Do Next dock control. It copies a placeholder `sk_sage_your_key_here` setup bundle, emits `status_first_request_setup_copied` with `operator-first-request-setup`, and lets the operator move the setup-copy activation KPI without sending activation email, exposing a real key, or changing billing/provider-resale state. When real activation sends or managed-provider resale are gated and founder-sales outreach copies are still zero, the same Do Next dock now renders a compact revenue-backup block with copy/open actions for the no-secret founder-sales first reply and next-revenue outreach, reusing `outreach_snippet_copied` / `outreach_email_draft_opened` telemetry without sending email or changing provider controls. When Cloudflare BIC token-scope copies are still zero, the Do Next dock also promotes a read-only BIC token-scope packet that records `status_cloudflare_bic_token_scope_copied` with `operator-cloudflare-bic-token-scope` and does not print token values, mutate Cloudflare, deploy, or disable Browser Integrity Check.
- The private launch funnel renders copyable `APPROVE_ACTIVATION_FOLLOWUP` and `HOLD_ACTIVATION_FOLLOWUP` decision records beside the no-secret approval packet, and promotes the same approve/hold handoff into a first-screen activation approval banner whenever the live next-best action is approval-blocked with no recorded decision copy. If auth hydration or queue changes make the last packet stale, the dashboard shows `packet_review_stale`, reports reviewed-recipient coverage, hides approve/hold as the current action, and keeps real-send command buttons disabled until a refreshed no-secret packet covers the live queue. The banner is audit-only: it does not send email, mutate queued follow-ups, expose customer data, or bypass the typed `SEND_ACTIVATION_FOLLOWUPS` gate.
- `/analytics/funnel` and the launch snapshot expose `activationApprovalDecisionCopies` and snippet buckets for those copied approve/hold decision records, separate from packet reviews and real sends, so operators can see whether the approval handoff happened before any protected send command is used.
...
- `https://sagerouter.dev` keeps copy-first activation in the first viewport and sticky footer: `Copy 60-second setup first`, `Create API key`, `Finish setup key`, and `One-subscription beta` stay visible so landing-page visitors can copy the hosted setup bundle before account creation, create the generated key, recover a no-key signup, or request private-beta managed-access review after scrolling without joining a waitlist. The signed-out activation nudge also exposes `Copy setup first`, records `quickstart_snippet_copied` with `landing-nudge-setup-bundle`, and switches its account CTA to the setup-attributed key handoff after copy. The first-screen managed-access review prompt now includes a one-field homepage review form that posts contact plus fixed allowlisted one-subscription qualification buckets to `/api/waitlist` and records `managed_access_contact_capture_landed`, `managed_access_quick_form_presented`, `managed_access_contact_handoff_prompted` with `landing-homepage-managed-access-contact-handoff`, guarded `managed_access_quick_form_focused`, guarded `managed_access_quick_form_started`, `managed_access_quick_request_submitted`, `managed_access_quick_request_received`, and failure/validation telemetry without logging email values in marketing events. It also offers same-screen `Copy contact request`, `Open email draft`, and direct `Email support` fallbacks measured as `managed_access_contact_packet_copied` or `managed_access_contact_draft_opened` with `landing-homepage-managed-access-contact-*` snippets, so buyer intent can become a contact handoff even when inline submission is blocked. It still records `landing_managed_access_clicked` and `managed_access_interest_clicked` before routing to `/managed-access`, and it offers a copyable no-secret one-subscription review packet measured as `managed_access_review_packet_copied` with `landing-one-subscription-review-packet`; it does not enable bundled provider resale or change the OpenRouter BYOK boundary.
- `https://sagerouter.dev/fusion` is the premium compound-model page. It documents `sage-router/fusion`, the `sage-router:fusion` server tool, `tool_choice: "required"`, Pro/Max gating, the `fusion_plan_required` upgrade path, a copy-first `Copy 60-second Fusion setup` bundle that records `quickstart_snippet_copied` with `fusion-setup-bundle`, and measured Max/one-subscription managed-access review CTAs.
...
- `https://sagerouter.dev/managed-access` is the managed-provider-access private beta and Max implementation intake page. It now leads with a first-screen one-field work-email form measured as `managed_access_quick_form_presented` with `first-screen-primary-email-form`, with `Copy contact request`, `Open email draft`, direct support email, and `Start 30-second review` fallbacks grouped below it so one-subscription and Max requests can become contactable before the anonymous intent buttons. A visible copy-first first-screen choice panel also records `managed_access_contact_handoff_prompted` with `managed-access-first-screen-contact-choice` and gives buyers source-attributed no-secret copy/email-draft paths before the work-email path or scroll. The form includes required Turnstile verification inline and collects contact plus allowlisted qualification buckets such as deployment preference, expected monthly routed request volume, provider access posture, target provider family, commercial preference, support need, and target launch window. The start control records the existing managed-access intent event with `first-screen-start-review`, then records privacy-safe contact-capture/focus telemetry without logging email values. The hero fallbacks, sticky handoff, and contact-decision panel reuse `managed_access_contact_packet_copied` and `managed_access_contact_draft_opened` paths with source-specific snippets, while the work-email action records focus with `managed-access-contact-decision-focus`, so anonymous one-subscription demand can become a contact handoff without adding new secret-bearing telemetry or enabling managed resale. The fast form records privacy-safe explicit contact-capture landing, presentation, focus, copied contact-request packet, and email-draft fallback events (`managed_access_contact_capture_landed`, `managed_access_quick_form_presented`, `managed_access_quick_form_focused`, `managed_access_contact_packet_copied`, and `managed_access_contact_draft_opened`) with only qualification buckets, so operators can distinguish operator CTA arrivals, page presentation, field engagement, clipboard fallback, mail-client fallback, validation failures, and received contact requests without logging email values. The fast and full review forms use JS validation instead of browser-native blocking, so empty or invalid work-email attempts still record validation telemetry and show the sticky copy/email fallback before the visitor bounces. Default managed-access page visits now get a persistent idle contact handoff instead of being counted as contact-capture landings; the handoff records `managed_access_contact_handoff_prompted` with `managed-access-default-idle-handoff` and lets visitors choose work email, copy request, or open draft without enabling managed resale. Explicit operator, intent, hash, and contact-capture campaign URLs still highlight the 30-second private review path, immediately record a `managed-access-contact-capture-landing-handoff` prompt, keep a `managed-access-explicit-route-fallback-handoff` prompt available if focus/start does not follow, and auto-focus the form. Operator contact-capture CTAs anchor directly to that fast form. Anonymous one-subscription/Max/private-deployment/gateway-migration intent clicks now reveal the no-email intent handoff immediately, tag the intent click with `managed-access-intent-handoff`, and let buyers copy the no-secret intent packet, open an email draft, or copy Max/BYOK setup before they submit contact details. The hosted pricing page also exposes a copy-first no-secret one-subscription review packet measured as `managed_access_review_packet_copied` with `pricing-one-subscription-review-packet`, so pricing traffic can share buyer review context before submitting contact details. The managed-access page offers a hero-level copy-first buyer review packet measured as `managed_access_review_packet_copied` with `managed-access-first-viewport-one-subscription-packet`, a secondary no-secret one-subscription review packet, a local email-draft share path that records `managed_access_review_packet_copied`, a copied contact-request fallback measured as `managed_access_contact_packet_copied`, and a copyable Max/BYOK setup bundle that records `quickstart_snippet_copied` with `managed-access-max-byok-setup`, so internal buyer sharing, contact handoff, and immediate key-first Max fallback become measurable demand without requiring immediate form submission or managed resale. The packet explicitly says it is a one-subscription review packet, not a public entitlement or enabled managed-provider resale offer. It renders the live public managed-access readiness checkpoint from `/pricing`, including missing controls, provider-family readiness, one-subscription ready/blocked families, BYOK-only boundaries, an ordered public-safe onboarding sequence, and max-safe plan thresholds without exposing actual provider costs; it also renders a public managed-access offer ladder for review-only Pilot, Pro add-on, and Max contract-floor candidates without changing Stripe prices or enabling provider resale. It explains that managed access still requires provider authorization, provider terms acknowledgment, an authorized provider allowlist, a configured provider cost model, and plan-margin checks before activation; it does not collect prompts, workflow text, provider credentials, OAuth tokens, generated API keys, private keys, cookies, raw provider responses, actual provider costs, or customer data. The private unit-economics preflight also reports the binding public plan and safe pricing/quota/add-on actions for failed plans, including a public-threshold-only managed-access pilot/add-on/private Max contract ladder, while still withholding the private provider cost and any derived private required price. Provider authorization outreach starts from the no-secret `scripts/configure_managed_provider_resale_readiness.sh --provider-outreach-packet` command or `docs/launch/execution/provider-authorization-outreach.md`, and one-subscription fixed-plan review starts from `docs/launch/execution/one-subscription-pricing-review.md`; both must use private provider/contact systems for evidence and cost details. After an operator actually uses or shares the terminal provider outreach packet, run `scripts/configure_managed_provider_resale_readiness.sh --record-provider-outreach` to record one aggregate `status_managed_provider_outreach_copied` event with snippet `operator-provider-outreach-packet`; that makes provider authorization work visible in the launch funnel, but still does not send email, acknowledge terms, stage authorization evidence, write provider costs, deploy Cloud Run, or enable managed resale. After an operator actually reviews the authorization packet, run `scripts/configure_managed_provider_resale_readiness.sh --record-authorization-review` to record one aggregate `status_managed_provider_authorization_review_copied` event with snippet `operator-provider-authorization-review-packet`; that records review work only and still does not stage authorization evidence, acknowledge terms, write provider costs, deploy Cloud Run, or enable managed resale. After an operator actually reviews the provider terms packet, run `scripts/configure_managed_provider_resale_readiness.sh --record-terms-review` to record one aggregate `status_managed_provider_terms_review_copied` event with snippet `operator-provider-terms-review-packet`; that records review work only and still does not acknowledge terms, stage authorization evidence, write provider costs, deploy Cloud Run, or enable managed resale.
- `/pricing` public metadata now exposes the same public-safe review packages as `publicLaunch.managedProviderAccess.oneSubscriptionPackaging`, `offerLadder`, and `oneSubscriptionReadiness.offerLadder`. The package rows publish only review price, monthly request count, derived max-safe provider-cost thresholds, and disabled/private-review gating flags; they do not publish actual provider costs, credentials, prompts, raw responses, or enabled resale entitlements.
- The managed-access offer ladder is now an actionable contact-capture surface: each review-only package has `Request this review` and `Copy package request` actions, records existing privacy-safe managed-access handoff/contact-packet events, and routes package interest back into the fast work-email review path. The pricing page also links each public-threshold-only package row directly to the managed-access review flow.
- Shared article/provider pages now add the same no-secret contact handoff to their inline one-subscription form: `Copy contact request` and `Open email draft` record `managed_access_contact_packet_copied` / `managed_access_contact_draft_opened` with `article-managed-access-*` snippets, giving article-origin anonymous managed-access interest a contactable fallback before the visitor reaches the full managed-access page.
- Managed-access intent buttons now promote anonymous one-subscription/Max/private-deployment/gateway-migration demand into the same contact-capture path as operator CTAs: they record `managed_access_contact_capture_landed`, reveal the no-secret handoff, scroll/focus the fast review form, and keep managed-provider resale gated until provider authorization, terms, cost, quota, and abuse-review checks pass.
- Managed-access operator/contact-capture routes also show a same-screen route nudge and re-run the work-email focus path on hash changes and browser page-restore events, so routed buyers land back on `#managed-access-quick-form` instead of only recording a form presentation.
- Managed-access private-beta pages now include real fastest-path first-screen, hero, and contact-decision `mailto:` links for `support@sagerouter.dev`, synchronized to the selected review intent and entered work email. These direct email links record only `managed_access_contact_draft_opened` with a snippet ID and keep the same no-secret/managed-resale-gated boundary as the existing copy and form paths.
- Managed-access page visits now schedule a bounded idle contact handoff when the visitor has not interacted, recording `managed_access_contact_handoff_prompted` with `managed-access-default-idle-handoff` and offering work-email, copied request, or email-draft actions without sending email or enabling managed resale.
...
- `https://sagerouter.dev/quickstart` is the hosted API first-request path. It leads with `Copy 60-second setup bundle`, an always-visible `Create API key next` account handoff that records whether setup was copied first, a $6 Lite generated-key handoff for lightweight pilots, and a `Finish setup key` recovery link that records `quickstart_key_recovery_clicked` for no-key signups. It then shows `OPENAI_BASE_URL=https://api.sagerouter.dev/v1`, generated `sk_sage_*` key setup, the `sage-router/frontier` profile, premium `sage-router/fusion`, curl, JavaScript, Python, and Codex examples, plus 401/402/429/503 troubleshooting.
- Shared marketing email activation forms insert `Copy setup first`, optional GitHub Pro OAuth, and `Finish setup key` recovery with `shared_email_key_recovery_clicked` telemetry so calculator, Fusion, integrations, API docs, and migration pages all feed the same generated-key recovery path. Public marketing, docs, status, support, pricing, billing, comparison, and article recovery CTAs route through `/setup-key-recovery` before the same-email app login handoff. The shared article activation dock also injects hero `Copy setup first` and `Finish setup key` CTAs beside Pro create-key buttons on article and provider pages, recording `content_article_snippet_copied` with `article-hero-setup-bundle` and `content_article_key_recovery_clicked`; its inline and sticky copy-first CTAs record `content_article_snippet_copied` with `article-inline-setup-bundle` or `article-sticky-setup-bundle` before switching to the setup-attributed `Create API key next` handoff. Article traffic now gets measured one-subscription review CTAs with `inline-one-subscription-review` and `sticky-one-subscription-review` states so high-intent content can feed anonymous `managedAccessDemand` buckets before contact submission, and the inline article offer can submit a one-field `managed-access` waitlist request through the guarded `/api/waitlist` path without enabling provider resale. The article form also records privacy-safe contact-capture landing, presentation, focus, and start telemetry before submission, so article readers become measurable managed-access prospects without logging email values in marketing events.
- `/setup-key-recovery` now keeps its generated-key account auto-handoff dedupe to a short timestamp cooldown in `sessionStorage`, not a permanent tab-session marker. That preserves loop protection while allowing repeated recovery visits to re-open account setup after the cooldown, which targets the observed recovery-view to key-create dropoff without sending email or exposing customer data.
- `https://sagerouter.dev/pricing` uses the same measurable activation pattern for buyer-intent traffic: `Copy 60-second setup bundle` records `quickstart_snippet_copied` with `pricing-full-setup-bundle`, Pro activation links use `start=create_key`, `Create API key next` stays visible before and after copy, and the first viewport now includes a lowest-friction Lite path that sends $6 buyers to `start=create_key` before checkout. Lite, Pro, Max, and Max/BYOK pricing handoffs now lead with `start=create_key` so every public plan creates the generated `sk_sage_*` setup artifact before checkout; those plan-card links emit `pricing_setup_next_clicked` or `pricing_key_activation_clicked` instead of checkout-first friction. The page also includes a one-field managed-access review request that posts only contact plus allowlisted waitlist qualification buckets, records pricing-page `managed_access_contact_capture_landed` / `managed_access_quick_form_presented` telemetry on form presentation, and records `managed_access_quick_request_*` telemetry on submission; it captures one-subscription or Max/BYOK demand without enabling managed provider resale before provider authorization, terms, cost model, and margin controls pass.
- `https://sagerouter.dev/compare/model-gateways` and `/compare/openrouter` keep comparison traffic on the same key-first activation track with copyable 60-second setup bundles, GitHub/email setup, `Create API key next`, `Finish setup key` recovery, `quickstart_snippet_copied`, and `gateway_compare_key_recovery_clicked` telemetry. The OpenRouter comparison also exposes a copyable no-secret one-subscription review packet measured as `managed_access_review_packet_copied` with `openrouter-comparison-one-subscription-review-packet`, while keeping OpenRouter positioned as BYOK unless separately authorized for managed access.
- `https://app.sagerouter.dev/login.html?start=create_key` is the no-key signup recovery bridge. It keeps same-email recovery first, shows the generated-key-before-checkout steps, embeds an inline same-email setup-link form, provides copy-first placeholder setup plus same-GitHub recovery inside the recovery panel, and exposes a first-panel `Open API key setup now` handoff plus a same-email/same-GitHub/signed-in identity choice panel measured by `login_key_recovery_identity_choice_shown` and `login_key_recovery_identity_choice_clicked`. It also keeps the static, fully attributed `Create API key next` handoff with `setup=login-key-recovery`, `source_surface=recovery`, and `next=generated-key` before JS rewrites the links. Recovery landings focus the same-email flow first and use a one-second account-setup auto-handoff only as an idle fallback that cancels when the user starts typing or clicks a recovery action. Manual account-setup clicks persist a local handoff marker and send `login_key_recovery_clicked` plus `login_key_recovery_account_setup_clicked` before navigation, while auto-handoff emits scheduled, paused, and redirected events so operators can separate passive recovery views, same-email recovery starts, identity-choice engagement, and users who clicked through toward API-key setup without storing email addresses.
- `https://app.sagerouter.dev/account.html` and `/analytics.html` expose same-email `Finish setup key` recovery beside account/API-access CTAs, recording `account_key_recovery_same_email_prompt_shown`, `account_auth_key_recovery_clicked`, and `analytics_key_recovery_clicked` so returning no-key users do not have to rediscover the login recovery URL from marketing pages. Signed-in no-key accounts now see the one-minute setup key panel before the checklist, marked with `data-conversion-step="signed-in-no-key-setup"`, so the manual `Create setup key now` fallback is first-screen and clearly separated from checkout.
- `https://sagerouter.dev/api-troubleshooting` is the no-secret diagnostic path for hosted 401/402/429/503 responses. It documents safe probes, `WWW-Authenticate`, `Retry-After`, `X-RateLimit-*`, `X-Quota-*`, account/pricing/status onboarding links, a measured `Finish setup key` recovery CTA, measured `Copy 401 setup snippet` and `Copy Codex 401 profile` actions that record `api_troubleshooting_setup_copied`, a one-subscription managed-access beta review CTA for customers blocked by BYOK/provider access, and the non-secret `apiKeyPrefix` plus placeholder `setupSnippet` / `codexSetupSnippet` without asking customers to paste prompts or credentials.
- `https://sagerouter.dev/setup-key-recovery` is the public no-key recovery destination for API 401 `keyRecoveryUrl` responses and operator signup-to-key follow-ups. Its first actionable control is now the inline same-email setup-link form so the next measurable event is `setup_key_recovery_magic_link_requested`, while app-managed email validation records `setup_key_recovery_email_validation_failed` instead of letting native browser validation swallow bad submit attempts; it also surfaces a first-screen signed-in fast path that records `setup_key_recovery_fast_account_clicked` or `setup_key_recovery_fast_account_link_copied` for visitors who already have an app session and should go straight to generated-key setup. The fallback account setup, app login, GitHub OAuth, setup-copy, troubleshooting, managed-access, and support actions stay below that form. It keeps account setup as the signed-in fallback, auto-hands API-auth recovery traffic (`utm_source=api-auth` plus `signup_to_key_recovery`) and explicit recovery traffic such as `setup=login-key-recovery` or `source_surface=recovery` into account setup with attribution preserved, and gives unattributed idle recovery views a slower cancellable account-setup handoff so passive visitors either reach generated-key setup or emit `setup_key_recovery_account_setup_paused` when they start same-email recovery. The signed-in account page keys its automatic setup-key attempt guard by plan, start action, auth/recovery state, setup source, source surface, and next-step context, so a stale attempt from one recovery path does not suppress a fresh same-account handoff. When account setup opens but the visitor is still signed out, it emits `account_key_recovery_signed_out_prompt_shown` so the launch funnel separates same-email/OAuth sign-in dropoff from API-key creation failures. If the signed-in account identity loads but `/account/api-keys` listing is unavailable, the recovery page records `account_key_recovery_key_list_load_failed` and tries setup-key creation directly instead of trapping the user in account loading. When a recovered signed-in account still has no generated key, the account page scrolls to and focuses the manual `Create setup key now` button and emits `account_key_recovery_manual_create_prompt_shown`, giving the funnel a measurable manual fallback before the user clicks `account_no_key_setup_create_clicked`. It offers direct GitHub OAuth recovery for signups that used the same GitHub identity, sends returning signups through the same-email or same-OAuth account flow, explains generated-key-before-checkout behavior, provides a placeholder setup bundle that can be copied without exposing secrets, and routes one-subscription demand to managed-access beta review without enabling managed provider resale before provider terms, authorization evidence, allowlist, cost model, and margin checks pass. Operator activation packets use this page as the primary CTA, with app-hosted same-email/password and GitHub/OAuth URLs retained as fallbacks.
- Account Supabase auth callbacks preserve the recovery attribution parameters (`setup`, `source_surface`, `next`, `auth`, provider override, and UTM fields) through magic-link, signup, and OAuth redirects, so returning no-key users keep the same key-first context after authentication and the auto-key/manual-create telemetry remains classifiable.
- Anonymous model API 401 responses include a measured `keyRecoveryUrl` (`utm_source=api-auth`) in addition to `accountUrl`, `pricingUrl`, `statusUrl`, `openaiBaseUrl`, `apiKeyPrefix`, a placeholder `setupSnippet`, `codexProfile`, and a placeholder `codexSetupSnippet`, so API-client failures can route returning no-key users into setup-key recovery, auto-open account setup for generated-key creation, and give CLI/Codex users a safe copyable next command.
...
- `https://sagerouter.dev/docs/codex` is the dedicated Codex CLI setup path. It shows hosted `https://api.sagerouter.dev/v1/`, local `http://127.0.0.1:8790/v1/`, and Tailnet `http://<tailnet-host>:8790/v1/` profiles using `wire_api = "responses"` and `sage-router/frontier`, with `Finish setup key` recovery for returning no-key signups.
...
3. Generate an `sk_sage_*` API key, copy the raw key while it is shown once, test it against `/v1/models`, send a first browser-side `sage-router/frontier` chat completion from the account page, and use the copyable OpenAI SDK, Codex CLI, Anthropic-compatible, or curl quickstart. The signed-in account page auto-creates the first generated setup key once per session when no active key exists; saved `start=checkout` activation intent then proceeds toward checkout after the setup artifact exists. A dedicated signed-in no-key setup panel keeps the key-creation action visible for recovery users who return without a key, keeps its copy button id distinct from the explanatory copy for stable browser targeting, and a persistent post-key activation panel keeps verification, first request, and Codex setup copy available after key creation without recording raw keys in funnel telemetry. The signed-out activation nudge also exposes a measured placeholder setup-copy action before GitHub or email setup, so setup intent can be captured before account creation.
...
For signed-in, verified users without a generated key, the primary account activation button creates the first `sk_sage_*` key directly instead of only scrolling to the key panel. The one-time raw key display remains unchanged, and checkout can still continue from the preserved `start=checkout` intent after key creation. After a key exists but hosted routing is still locked, the post-key activation panel promotes the selected Lite/Pro/Max checkout action before the first-request button so unpaid keys do not dead-end on a predictable 402.
...
The account page also shows current-period usage from the same Supabase usage counter that the public edge enforces, including requests used, remaining monthly quota, the active request-per-minute limit, and upgrade recommendations when routing is blocked or usage passes 75%/90% of the current plan quota. The public edge publishes only safe enforcement metadata on `/edge/health` so launch readiness can verify Supabase auth, rate limits, pre-auth generated-key attempt throttling, durable quotas, immediate generated-key revocation, and non-wildcard browser CORS without exposing secrets. The built-in API key test calls the public edge's `/v1/models` endpoint with the generated key so a new customer can separate key, billing, quota, and backend availability problems before configuring an agent. The same account page can send a first browser-side `sage-router/frontier` chat completion with the session-only generated key, so users can prove paid routing works before copying client configuration. The support page gives those same customers a safe escalation path for account, billing, quota, generated-key, 401/402/429/503, reliability, security, and abuse issues without asking them to paste secrets into public channels. The signed-in account page renders a copyable safe support context packet with only plan, routing, quota, generated-key count, verification, first-request, endpoint, and support-path state; it omits prompts, provider credentials, OAuth tokens, generated API keys, private keys, cookies, raw provider responses, and customer data. The account launch checklist mirrors the `$10k MRR` activation funnel by marking signed-in account, generated key, paid routing, public-edge verification, and server-recorded first routed usage as separate steps. The hosted analytics dashboard at `https://app.sagerouter.dev/analytics.html` uses the signed-in account session and calls `/account/analytics`, so customers see only their own privacy-safe routing telemetry while `/analytics` and `/analytics/funnel` remain operator/global endpoints; it also reads the server-derived `/account/usage.activation` state for plan, usage, generated-key, first-request, quota, and routing status to show the next conversion action before or after checkout. Operators can view the private global launch funnel and operator customer review at `https://app.sagerouter.dev/launch-funnel.html` by entering the private admin or analytics token; the browser stores that token only in tab-scoped `sessionStorage` when explicitly requested. The launch funnel endpoint reports waitlist, managed-access beta interest, target provider family, commercial preference, support need, target launch window, inbound intent demand, `managedAccessDemandConversion` for anonymous-to-contact managed-access review gaps, and `managedAccessDropoff` for aggregate contact-capture presentation/focus/start/submit/received rates, plus signup, generated-key, first-request, paid-conversion, retained-paid, estimated MRR, target attainment, target-aware bottlenecks, conversion actions with owner/surface/success metric, checkout-readiness friction from aggregate checkout unavailable, checkout failure, billing-portal failure, and manual-settlement failure events, anonymous marketing CTA event/plan/source/channel breakdowns, model catalog family/search-bucket demand, browser-visible auth provider state checks for OAuth onboarding, ranked acquisition actions, per-plan `$10k MRR` gaps, prioritized plan revenue actions, with `planRevenueActions` mirrored at the top level and under `mrr.planRevenueActions`, and compact `managedProviderReadiness` for one-subscription readiness blockers without returning email addresses, prompts, message bodies, API keys, provider credentials, OAuth tokens, actual provider costs, raw campaign URLs, raw model search text, or raw responses. The managed-access intake also emits anonymous page-view, fast-form focus, email-draft fallback, form-start, validation-failure, submit, and received events with only allowlisted qualification buckets, so one-subscription and Max implementation demand drop-off is visible before full contact submission while email and company fields remain confined to the waitlist path. When there are no qualified source/channel clicks yet, the same endpoint seeds zero-click acquisition actions for Gateway migration, GitHub builder traffic, pricing checkout proof, calculator qualification, model catalog activation, quickstart first-request activation, managed-access beta outreach, and launch-plan outreach so the operator view still points at concrete launch motions. The private launch funnel renders deterministic campaign links for those ranked acquisition actions, using only coarse action buckets to generate public UTM URLs rather than replaying raw visitor URLs, and each link can be copied from the dashboard for founder-sales outreach. It also renders the `managedAccessDemandConversion` status, priority, contactable lead gap, dominant demand buckets, the `managedAccessDropoff` status/rates/action, fast-form focus and email-draft counts, a first-screen managed-access contact-capture handoff, a copyable no-secret contact-capture packet, a no-secret mailto draft for that packet, `/managed-access` contact-capture CTA, and a founder-sales first-reply/draft backup when founder-sales outreach is also unworked, so anonymous one-subscription demand can be converted into review requests or warm founder-led outreach before managed resale is enabled; copying the packet records `operator_managed_access_packet_copied` with snippet `managed-access-contact-capture-packet`, opening the dashboard draft records the same event with snippet `managed-access-contact-capture-email-draft`, and the founder-sales backup records existing `outreach_snippet_copied` / `outreach_email_draft_opened` events without sending email. The operator customer review calls `/admin/customers` through the control-plane edge and shows bounded customer, usage, activation, review flags, operator audit events, and public API-key metadata without raw generated keys, API-key hashes, provider credentials, prompts, or raw responses.

Founder-sales kit, private launch-funnel, and status-page founder fallback copy actions use managed-access review links anchored to `#managed-access-quick-form` so outreach lands on the contact-capture path, and they roll into `/analytics/funnel.marketingIntent.founderSalesOutreachCopies` and `founderSalesOutreachCopiesBySnippet`, so Lite/Pro/Max and one-subscription direct outreach is visible as a first-class launch metric instead of only raw event rows. Managed-access review and provider-authorization packet copies roll into `marketingIntent.managedAccessPacketCopies` and `managedAccessPacketCopiesBySnippet`, so provider authorization work is visible separately from anonymous managed-access demand clicks.

The hosted operator dashboard also includes an operational readiness panel that reads public `/edge/health` and `/pricing` metadata to show live edge health, API-key enforcement, Stripe checkout/portal readiness, and managed-provider launch gating without using the private token or exposing secrets. The public status page mirrors the safe subset of that posture for customers: checkout readiness, activation-email configured-vs-copy-fallback state, managed-access gating, and Browser Integrity Check guidance for raw API clients. Managed-provider readiness includes ordered `nextActions` derived from `missingControls` plus an `onboardingSequence` for provider authorization, terms/public-control review, private unit-economics review, and final explicit enablement hold, so pricing, status, and operator launch packets can name both the immediate blocker and the safe onboarding order without exposing provider credentials, authorization references, or actual provider costs. The private launch funnel also renders a dedicated managed-access readiness panel with a copyable no-secret approval packet, terms packet command, provider outreach packet command, resale dry-run, unit-economics preflight, staging command, one-subscription ready/blocked families, BYOK-only boundary, prerequisite URLs, public-safe onboarding sequence, and public plan threshold rows; it excludes provider credentials, authorization-reference values, actual provider costs, prompts, raw provider responses, generated API keys, and customer data. The unit-economics preflight names the binding fixed plan and recommends public pricing, quota, add-on, or exclusion actions when a private cost candidate fails, but it still does not print the candidate cost, gross-margin calculation, or a derived private required price. It also renders a copyable no-secret operator launch brief that condenses the current `$10k MRR` snapshot, top conversion move, revenue motions, acquisition links, model catalog demand, managed-access demand, checkout friction, recovery-auth starts, no-key signup follow-up count/CTA from `activationFollowUps`, and GitHub OAuth onboarding state for founder-sales and support follow-up without emails, customer IDs, prompts, OAuth tokens, generated API keys, provider credentials, raw campaign URLs, raw model search text, or raw responses. `/analytics/funnel` also exposes `activationFollowUps.sendableQueued`, `reviewOnlyQueued`, `unknownQueued`, and segment lists, plus an aggregate `operatorExecutionPacket` with signup-to-key recovery URLs, segment counts, draft subject/body, telemetry event names, and privacy flags so dashboards, agents, and runbooks can execute the current activation action without scraping browser copy. The operator dashboard renders a no-secret activation approval packet and approval checklist that separates sendable segments from review-only auth-repair segments before any real send command is copied. The same server-derived `activationApprovalReadiness.decisionChecklist` is exposed to the dashboard, CLI approval packet, and agents, covering no-secret packet review, dry-run coverage, review-only segment exclusion, hydratable auth signups without customer rows, existing missing-auth-user account-link review/exclusion, next-segment-only approval, and typed-confirmation protection. The aggregate `operatorExecutionPacket.authRepair` handoff includes only counts, segment names, hydrate candidate counts, account-link review counts, the hydrate endpoint only when it can create missing customer rows, a dry-run account-link repair command that can apply only with `REPAIR_AUTH_LINKS`, a bounded safe customer-review command for stale account links or hydration no-ops, and privacy flags, so operators can repair or explicitly exclude `missing_auth_user` rows and refresh the funnel before approving any additional real sends. The operator-only no-key signup queue pulls bounded customer review data from `/admin/customers`, shows generated-key-first follow-up links, and supports per-customer or batch snippet copy so activation outreach can be executed without exposing raw keys, key hashes, prompts, provider credentials, or provider responses. If `SAGE_ROUTER_ACTIVATION_EMAIL_FROM` and `SAGE_ROUTER_RESEND_API_KEY` are configured, the private `POST /admin/customers/send-activation-followups` endpoint can send the same generated-key-first recovery drafts through Resend; without those env vars it fails closed or can be used as a dry run. Public `/pricing` exposes a reduced, secret-free `activationEmailReadiness` object for the status page, while `/analytics/funnel` exposes full secret-free operator command templates in `activationFollowUps.emailReadiness` and `operatorExecutionPacket.emailReadiness` so operators can see whether real sending is configured or whether the copy/mailto fallback is the current path before clicking Send. Copying those snippets records privacy-safe `operator_no_key_followup_*`, `operator_execution_packet_copied`, `operator_managed_access_packet_copied`, and `operator_managed_access_command_copied` funnel events, and `/analytics/funnel` rolls no-key outreach into `activationFollowUps.operatorFollowUpCopies` so queued follow-ups and worked follow-ups are visible separately. Operator send telemetry separates unique sendable dry-run coverage from raw recorded dry-run recipient counts, so repeated all/segment dry-runs can prove `verified`/`unverified` segment coverage without overstating unique recipients. Browser-triggered real sends also require the operator to type `SEND_ACTIVATION_FOLLOWUPS` after explicit approval, in addition to the private token, trusted origin, and server-side confirmation field.

The private launch-funnel dashboard resolves activation real-send commands from
`activationFollowUps.emailReadiness.segmentCommandTemplates` first, then falls
back to the legacy flat `sendCommandTemplate`. That keeps the copied handoff
aligned with the approved `verified`, `unverified`, or `all` segment while the
existing approval packet, freshness window, private token, trusted origin, and
typed confirmation gates remain unchanged.
...
operator token, trusted Sage Router browser origin, browser confirmation, and
typed dashboard approval phrase `SEND_ACTIVATION_FOLLOWUPS`. Dry runs do not
require the confirmation field or typed approval phrase, so operators can verify
queued counts, segments, and plan mix before a configured sender can deliver
email. The approval decision checklist is aggregate-only and must not contain
emails, customer IDs, generated keys, prompts, OAuth tokens, provider
...
`Create API key`, and `Finish setup key` visible after scrolling; the setup copy records
`quickstart_snippet_copied` with `landing-sticky-setup-bundle` before handing off to account
key creation. The signed-out activation nudge now offers `Copy setup first`, records
`landing-nudge-setup-bundle`, and changes its account handoff to `Create API key next` after
copy so nudge views can move the setup-copy KPI. It keeps pricing, quickstart, status, model gateway comparison, model catalog,
...
Run `scripts/configure_cloudflare_api_bic_skip.sh --operator-packet` for a
read-only reliability handoff that combines the active-token check, local
token-candidate audit summary, required permissions, and next commands without
...
Its `Founder Sales Fallback` section now prints the no-secret founder-sales
packet command, recommended first-reply/next-revenue copy path, and
post-use-only recording command so Lite/Pro/Max outreach can move while
activation sends and managed resale stay gated.
...
the API-auth and operator generated account handoff URLs, and the smoke-only
...
`$10k MRR` state and top conversion move, consume `activationQueue` for
...
for review-only signup rows; it reports checked, eligible, updated, skipped,
...
After the operator actually chooses a no-secret decision line from the private
launch funnel or terminal approval packet, run
...
packet and `SEND_ACTIVATION_FOLLOWUPS` typed-confirmation gate. Do not record
approve/hold decisions from the public status page; it only links back to the
private decision gate because public `/pricing` cannot verify live packet
coverage.
