...
if [[ "${DEPLOY_FROM_GHCR_REMOTE}" == "1" || -n "${CLOUD_RUN_IMAGE}" || -n "${GHCR_IMAGE_DIGEST}" ]]; then
  ...
  gcloud run services update "${SERVICE_NAME}" \
    ...
    --platform managed \
    --timeout 900 \
    --update-env-vars '^|^SAGE_ROUTER_OLLAMA_TIMEOUT_SECONDS=300|SAGE_ROUTER_OPENAI_TIMEOUT_SECONDS=180|SAGE_ROUTER_ANTHROPIC_TIMEOUT_SECONDS=180|SAGE_ROUTER_GOOGLE_TIMEOUT_SECONDS=180|SAGE_ROUTER_OPENCLAW_TIMEOUT_SECONDS=900|SAGE_ROUTER_OPENCLAW_CODE_TIMEOUT_SECONDS=900|SAGE_ROUTER_AUDIO_PROXY_TIMEOUT_SECONDS=300'
