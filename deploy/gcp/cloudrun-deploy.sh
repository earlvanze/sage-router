...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --concurrency 20 \
  --timeout 300 \
  --set-env-vars '^|^SAGE_ROUTER_HOME=/app|SAGE_ROUTER_BUNDLED_OLLAMA_AUTOSTART=0|SAGE_ROUTER_OLLAMA_AUTO_PULL_PATTERNS=|SAGE_ROUTER_PROFILE_OVERLAYS=ollama|SAGE_ROUTER_MAX_PROVIDER_ATTEMPTS=2|SAGE_ROUTER_OLLAMA_TIMEOUT_SECONDS=120|SAGE_ROUTER_MAX_ACTIVE_API_KEYS_PER_CUSTOMER=5|SAGE_ROUTER_FIRESTORE_PROJECT_ID='"${PROJECT_ID}"'|SAGE_ROUTER_SUPABASE_URL=https://awtangrlqqsdpksarhwo.supabase.co|SAGE_ROUTER_SUPABASE_AUTH_ENABLED=1|SAGE_ROUTER_SUPABASE_MIRROR_ENABLED=1|SAGE_ROUTER_CLIENT_AUTH_REQUIRED=1|SAGE_ROUTER_PUBLIC_BASE_URL='"${PUBLIC_BASE_URL}"'|SAGE_ROUTER_MARKETING_BASE_URL='"${MARKETING_BASE_URL}"'|SAGE_ROUTER_APP_BASE_URL='"${APP_BASE_URL}"'|SAGE_ROUTER_API_BASE_URL='"${API_BASE_URL}"'|SAGE_ROUTER_CORS_ORIGIN='"${CORS_ORIGIN}"'|SAGE_ROUTER_STRIPE_PRICE_IDS='"${STRIPE_PRICE_IDS}" \
