...
SECRET_MAPPINGS=(
  ...
  "SAGE_ROUTER_SUPABASE_ANON_KEY=AOPS_SUPABASE_ANON_KEY"
  "SAGE_ROUTER_SUPABASE_SERVICE_ROLE_KEY=AOPS_SUPABASE_SERVICE_ROLE_KEY"
...
for mapping in "${SECRET_MAPPINGS[@]}"; do
  ...
  if gcloud secrets describe "${secret_name}" --project "${PROJECT_ID}" >/dev/null 2>&1; then
    SET_SECRETS+=("${env_name}=${secret_name}:latest")
  else
    echo "Skipping missing Secret Manager secret: ${secret_name} -> ${env_name}" >&2
  fi
done
