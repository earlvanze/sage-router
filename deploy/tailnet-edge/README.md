...
- `/edge/health` reports the selected upstream, last probe latency/error for every configured upstream, auth mode, and public-safe enforcement state for rate limits, durable quotas, generated-key auth-cache TTL, and API-key prefix.
