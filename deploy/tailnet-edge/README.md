...
- `/edge/health` reports the selected upstream, last probe latency/error for every configured upstream, auth mode, public-safe failover policy (`lowest-latency-healthy`, retry-enabled statuses, retry header, and healthy upstream count), and public-safe enforcement state for rate limits, auth-attempt throttling, durable quotas, generated-key auth-cache TTL, and API-key prefix.
